...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const authCheck = SecurityGuard.authorize(locals.user.role, 'REGISTER_SUPRIMENTO');
  ...
  try {
    ...
    if (!shiftId || !amountCents || !description) {
      return json({ success: false, error: 'Preencha shiftId, valor do suprimento e justificativa.' }, { status: 400 });
    }
    ...
    return json({ success: false, error: `Erro ao registrar suprimento: ${err.message}` }, { status: 500 });
  }
};
